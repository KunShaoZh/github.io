...
public class LinkList<T> implements Iterable<T>{
    ...
    /**
     * 单向链表翻转(递归调用直到最后一个结点)，通过定义指针来实现，第三者角度
     * @param cursor 当前指针指向的结点
     * @return 当前结点
     */
    private Node<T> reverse(Node<T> cursor) {
        // 是否为链表的最后一个结点
        if (cursor.next == null) {
            // 如果是最后一个结点，那么头结点指向最后一个结点
            head.next = cursor;
            return cursor; // 返回最后一个结点
        }
        // 当前结点的下一个结点
        Node<T> next = reverse(cursor.next); //
        next.next = cursor; // 下一个结点的next指向当前结点
        cursor.next = null;// 给当前结点的下一个结点置为null，这里当前结点下一个结点一定要置为null，否则最后一次递归会造成两个结点间的环链表
        return cursor;
    }
    ...
    public void reverse2() {
        if (head.next == null) {
            ...
            this.head.next.reverse(this.head);
        }
    }
